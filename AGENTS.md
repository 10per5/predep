...
| `predep/src/logger/prompter.h/cpp`        | Prompter base + InteractivePrompter/ForcePrompter/NullPrompter + factory; user confirmation prompts               |
...
| `predep/src/action/action.h`              | Base class: `is_resolved()`, `resolve()`, `check_outputs()`, `resolve_cwd()`, `confirm_build_context()`            |
...
a message suggesting `--force`. The `--force` flag is declared in CLI args but
behavior is not yet wired up.

### Shared Action Helpers

The `action` base class (`src/action/action.h`) provides three static helpers:

- `resolve_build_context(sd, ctx)` — returns the effective build_context value
  after resolving stage-level + platform override
- `resolve_cwd(sd, ctx)` — computes the working directory path
- `confirm_build_context(sd, ctx, error)` — shows warning and prompts for
  confirmation; returns false with error if user rejects or non-interactive
...
| `ForcePrompter`       | Auto-accepts (for `--force`)      |
