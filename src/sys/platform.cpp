...
std::string cache_dir()
{
    ...
    return local ? std::string(local) + "/predep" : "~/.cache/predep";
#elif __APPLE__
    auto *env = std::getenv("PREDEP_CACHE");
    return env ? std::string(env) : "~/Library/Caches/predep";
#else
    auto *env = std::getenv("PREDEP_CACHE");
    return env ? std::string(env) : "~/.cache/predep";
#endif
}
...
#ifdef _WIN32
    auto *local = std::getenv("LOCALAPPDATA");
    return local ? std::string(local) + "/predep" : "~/.local/share/predep";
#elif __APPLE__
    auto *env = std::getenv("PREDEP_DIR");
    return env ? std::string(env) : "~/Library/Application Support/predep";
#else
    auto *env = std::getenv("PREDEP_DIR");
    return env ? std::string(env) : "~/.local/share/predep";
#endif
}
