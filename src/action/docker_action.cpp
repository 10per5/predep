...
bool docker_action::resolve(stage_desc &sd, runtime &ctx, std::string &error)
{
    ...
    std::vector<std::pair<std::string, std::string>> steps = {
        {"docker build -t " + tag + " -f " + recipe + " .", "docker build failed for " + sd.name},
    ...
    for (auto &[cmd, err] : steps)
    {
        if (ctx.logger)
            ctx.logger->info(cmd);
        if (process::run(process::shell(), {process::shell_cmd_flag(), cmd}) != 0)
        {
            error = err;
            return false;
        }
    ...
    bool ok = process::run(process::shell(), {process::shell_cmd_flag(), cp_cmd}) == 0;
    if (!ok)
        error = "docker cp failed for " + sd.name;

    process::run(process::shell(), {process::shell_cmd_flag(), "docker rm " + container + " >/dev/null"});
    return ok;
}
